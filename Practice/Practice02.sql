...
select date_format( max(hire_date), '%Y년 %m월 %e일' )  '신입입사날'
from employees
;
...
select job_id
     ...
     , max(salary) '최고임금' 
     , min(salary) '최저임금'
     -- , sum(salary)-min(salary) '(평균임금 – 최저임금)'
from employees
group by job_id
...
select date_format( min(hire_date), '%Y-%m-%e %W' )  '축하합니다'
from employees
;
...
select date_format( min(hire_date), '%Y년 %m월 %e일(%a)' )  '축하합니다'
from employees
;
